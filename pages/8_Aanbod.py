...
st.markdown(
    ...
    <style>
      ...
      [class*="st-key-offer_section_"] > div {
        margin-top: 1.45rem;
        padding: 1.25rem;
      ...
      .st-key-offer_card_step > div {
        border-color: #c9d8dc !important;
        background: #f9fcfd !important;
        box-shadow: 0 8px 18px rgba(20, 66, 78, 0.09);
      }

      .st-key-offer_card_soft > div {
        background: #f3f9fa !important;
      }

      [class*="st-key-offer_card_"] > div,
      .st-key-offer_card_step > div,
      .st-key-offer_card_soft > div {
...
with st.container(key="offer_hero", border=True):
...
with st.container(key="offer_section_choose", border=True):
    ...
    with left:
        with st.container(key="offer_card_step", border=True):
    ...
    with right:
        with st.container(key="offer_card_ramp", border=True):
...
with st.container(key="offer_section_deep", border=True):
    ...
    left, right = st.columns(2, gap="large")
    with left:
        with st.container(key="offer_card_vt1", border=True):
    ...
    with right:
        with st.container(key="offer_card_cp", border=True):
...
with st.container(key="offer_section_submax", border=True):
    st.markdown('<h2 class="offer-section-title">Submaximale insteek (indien nodig)</h2>', unsafe_allow_html=True)
    with st.container(key="offer_card_soft", border=True):
...
with st.container(key="offer_section_moments", border=True):
    ...
    for col, (title, text) in zip(cols, moments):
        with col:
            with st.container(key=f"offer_card_moment_{title.replace(' ', '_').replace('-', '_').replace('(', '').replace(')', '')}", border=True):
                st.markdown(f"<h3 class='offer-card-title'>{title}</h3>", unsafe_allow_html=True)
                st.write(text)

with st.container(key="offer_section_prices", border=True):
...
with st.container(key="offer_section_included", border=True):
...
with st.container(key="offer_section_plan", border=True):
